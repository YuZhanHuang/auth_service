...
mail = Mail()
migrate = Migrate(directory=os.path.join(root_dir, 'migrations'))
redis = StrictRedis(host=os.environ.get('REDIS_HOST', 'redis'),
...
class Service:
    ...
    def create(self, **kwargs):
        ...
        self.dispatch_model_event('pre_create', instance=model)
        instance = self.save(model)
        self.dispatch_model_event('created', instance=instance)
        return instance
    ...
    def dispatch_model_event(self, name, **kwargs):
        """觸發模型事件"""
        event = '{model}_{event}'.format(
            model=self.__model__.__tablename__.lower(), event=name)
    ...
    def update(self, model, **kwargs):
        ...
        for k, v in self._preprocess_params(kwargs).items():
            setattr(model, k, v)
        self.dispatch_model_event('pre_update', instance=model)
        instance = self.save(model)
        self.dispatch_model_event('updated', instance=instance)
        return instance
    ...
    @staticmethod
    def transform_query_value(field, value):
        return field, value

    def to_filter(self, filters, tz=LOCAL_TZ):
        keys = {
        ...
        for attr, value in filters.items():
            ...
            field, value = self.transform_query_value(field, value)
...
class Reviewable(metaclass=abc.ABCMeta):
    """Interface for reviewable"""

    @abc.abstractmethod
    def on_approved(self, review, instance=None, **kwargs):
        raise NotImplementedError()

    @abc.abstractmethod
    def on_denied(self, review, instance=None, **kwargs):
        raise NotImplementedError()
